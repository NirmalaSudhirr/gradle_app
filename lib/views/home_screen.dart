...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget buildSummaryBar(BuildContext context, {bool desktop = false}) {
    return Obx(() {
      ...
      for (final code in ctrl.scannedCodes) {
        ...
        if (parts != null) {
          total += parts.length;
          ok += parts.where((p) => p.status.trim().toLowerCase() == 'ok').length;

          // Debug: print each code's parts with status
          for (var p in parts) {
            print('Code: $code, Part: ${p.characterName}, Status: "${p.status}"');
          }
        }
      }

      print('SummaryBar total parts: $total, ok parts: $ok');
      ...
      double progressValue = (total == 0) ? 0.0 : (ok / total).clamp(0.0, 1.0);

      return Card(
        child: Padding(
          padding: EdgeInsets.symmetric(horizontal: desktop ? 22 : 14, vertical: desktop ? 20 : 12),
          child: Column(
            ...
            children: [
              Row(
                ...
                children: [
                  Text(
                    'Overall',
                    style: GoogleFonts.inter(fontWeight: FontWeight.w600, fontSize: desktop ? 18 : 16),
                  ),
                  Container(
                    ...
                    child: Text(
                      label,
                      style: GoogleFonts.inter(color: barColor, fontWeight: FontWeight.w600, fontSize: desktop ? 15 : 14),
                    ),
                  ),
                ],
              ),
        ...
        return Card(
          ...
          child: Container(
            child: Obx(() {
              ...
              // This is the FIX: entire content is vertically scrollable now.
              return SingleChildScrollView(
                child: Padding(
                  padding: EdgeInsets.only(
                      top: MediaQuery.of(context).size.height * (desktop ? 0.02 : 0.03)
                  ),
                  child: Column(
  ...
  Widget buildQrCard(BuildContext context, {bool desktop = false}) {
    ...
    // Move controller to state if possible. Here for demonstration:
    final TextEditingController textCtrl = TextEditingController(
      text: ctrl.inputText.value,
    );
            ...
            children: [
                        ...
                        SizedBox(
                          ...
                          child: Obx(
                              ...
                              style: ElevatedButton.styleFrom(
                                ...
                                backgroundColor:
                                ctrl.inputText.value.trim().isNotEmpty
                                    ? Colors.green[700]
                                    : Colors.grey,
                                foregroundColor: Colors.white,
                                minimumSize: Size(54, buttonHeight),
                              ),
                              onPressed:
                              ctrl.inputText.value.trim().isNotEmpty
                                  ? () async {
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  onPressed: () {
                    // Get the currently selected parts for the last scanned code
                    if (ctrl.scannedCodes.isNotEmpty) {
                      final code = ctrl.scannedCodes.last;
                      final parts = ctrl.partsMap[code] ?? [];
  ...
  void downloadCsv(List<CharacterStatus> parts) {
    final buffer = StringBuffer();

    // Add CSV headers
    buffer.writeln('Character Name,Status,Date,Time');

    // Add rows
    for (var part in parts) {
      // Escape commas or quotes if necessary
      final characterName = '"${part.characterName.replaceAll('"', '""')}"';
      final status = '"${part.status.replaceAll('"', '""')}"';
      final date = '"${part.date.replaceAll('"', '""')}"';
      final time = '"${part.time.replaceAll('"', '""')}"';

      buffer.writeln('$characterName,$status,$date,$time');
    }

    final csvData = buffer.toString();

    // For Flutter web, use `dart:html` to trigger download
    final bytes = utf8.encode(csvData);
    final blob = html.Blob([bytes], 'text/csv');
    final url = html.Url.createObjectUrlFromBlob(blob);
    final anchor = html.document.createElement('a') as html.AnchorElement;
    anchor.href = url;
    anchor.download = 'parts_report.csv';
